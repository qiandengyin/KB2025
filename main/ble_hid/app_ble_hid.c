...
static bool g_ble_is_inited = false;

#define CHAR_DECLARATION_SIZE (sizeof(uint8_t))

static void hidd_event_callback(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param);

#define HIDD_DEVICE_NAME "CursorKeyboard_BLE"
...
void app_ble_hid_init(void)
{
    ...
    g_ble_is_inited = true;
}

static bool app_bel_hid_is_connected(void)
{
    return g_ble_conn;
}
...
void app_ble_hid_send_key(uint8_t *data, uint8_t len)
{
    ...
    uint8_t key_num = 6;
    uint8_t special_key_mask = key_buffer_last[0];
    uint8_t key_cmd[8] = {0};
    memcpy(key_cmd, key_buffer_last + 2, len - 2);
    esp_hidd_send_keyboard_value(g_ble_conn_id, special_key_mask, key_cmd, key_num);
}
